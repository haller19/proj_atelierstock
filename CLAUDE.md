...
// inputQty:    使用した原材料の量（parseFraction で小数・分数・%を受け付ける）
// outputs:     切り出し後の部品IDと生成数量の配列（inputPartId の子部品を先頭に表示）
...
// cat: 完成品のカテゴリ（ピアス・イヤリング・ネックレス等。任意）
...
| タブID | 表示名 | 内容 |
|--------|--------|------|
| `dashboard` | HOME | 今月KPI（クリックで売上タブへ遷移）・チャネル別売上棒グラフ（クリックでフィルター遷移）・在庫アラート＋在庫補充ボタン |
| `parts` | 部品在庫 | 部品一覧（加重平均単価・仕入先・在庫ステータス）、原材料の残量バー、原材料→加工済み部品の親子インデント表示、編集・＋で部品登録 |
| `prodstock` | 完成品 | カテゴリフィルタ＋ソート、サブタブ「在庫」「レシピ・原価」、レシピ編集・削除、＋で制作記録またはレシピ登録 |
| `records` | 仕入・廃棄 | サブタブ「仕入記録」「廃棄記録」「加工記録」「部品マスタ」、各記録の編集・削除。加工記録は原材料ごとにグループ表示 |
...
- `type:"part"`（加工済み部品）に `parentId` を設定すると、親の原材料と連携
- 品番は親登録時に `{親の品番}-001` の連番で自動付与（手動変更可）
- 部品在庫一覧：原材料の直下にその加工済み部品をインデント表示（左ボーダー＋「↳ 親名 の加工済み部品」ラベル）
- 仕入モーダル：`type:"part"` の部品は選択不可（加工記録から在庫が増える）

### 加工記録の入力UX

原材料の使用量・ロス量入力は「測りにくい単位」（m²・cm等）になるため、以下の補助UIを設ける。
...
**切り出し結果セレクト**
- 原材料を選択すると、その原材料の子部品（`parentId` 一致）を optgroup で先頭に表示
- `<optgroup label="↳ この原材料の加工済み部品">` → `<optgroup label="その他の加工済み部品">`
...
**加工記録一覧の表示**
- 原材料IDでグループ化し、グループヘッダーに「{親素材名 →} {原材料名} #品番 N件」を表示
- 親素材を持つ原材料を先にソート（`parentId` 有→無の順）

### 完成品カテゴリ

- `productCats` = `["ピアス","イヤリング","ネックレス","ブレスレット","リング","その他"]` ＋ 動的追加分
- レシピ登録モーダルでチップ選択（トグル：再タップで解除）
