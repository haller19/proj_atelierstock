...
- **部品在庫** — 部品マスタの登録・編集、加重平均単価・仕入先・在庫ステータス表示。母材→中間材の親子管理（品番連番・残量バー・インデント表示）。**カードビュー/テーブルビュー切り替え**対応（テーブルビューはセルをクリックしてインライン編集、Tab/Shift+Tab でセル間移動、最終行から新規部品を直接追加可能）
...
- **売上** — 年度・チャネルフィルタ、チャネル別手数料・純利益計算。受注（オーダー品）売上は使用部品を選択して在庫差し引き・原価反映
- **データ管理** — JSON/CSV エクスポート・インポート（管理設定から操作）
- **Google Drive 同期** — ユーザーごとの Google Drive にデータを保存。複数端末で自動同期。ヘッダーにステータス表示（待機中/同期中/同期済み）
- **全体設定** — 管理設定→全体設定から加重平均単価の税込み/税抜き表示を切替。原価計算・純利益計算にも反映
...
- **セッション永続化**: 一度サインインすると、次回以降はページリロード・ブラウザ再起動後も自動で再接続（Google アカウントにサインイン中の場合）
